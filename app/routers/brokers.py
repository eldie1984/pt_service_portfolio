    ...
    for attempt in range(max_retries):
        try:
                ...
                result = await db.fetchrow(
                    ...
                    broker.name, broker.type, broker.commision, broker.is_active,
...
async def delete_broker(broker_id: str, db=Depends(get_db_conn)):
    ...
    try:
        ...
        broker = await db.fetchrow(
            "SELECT id FROM portfolio_service.brokers WHERE id = $1", 
            broker_id
        )
        ...
        # Delete broker
        await db.execute(
            "DELETE FROM portfolio_service.brokers WHERE id = $1",
            broker_id
        )
