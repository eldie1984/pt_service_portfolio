...
async def get_exchanges(
    type: Optional[str] = Query(None, description="Filter exchanges by type (stock, crypto, bond, commodity, forex)"),
    ...
    for attempt in range(max_retries):
        try:
            ...
            if "duplicate key" in str(e):
                ...
                raise HTTPException(status_code=409, detail="Exchange with this code already exists")
            elif "pool is closed" in str(e) and attempt < max_retries - 1:
                logger.warning(f"Database pool closed, retrying (attempt {attempt + 1}/{max_retries})")
...
async def delete_exchange(exchange_id: str, db=Depends(get_db_conn)):
    ...
    try:
        ...
        exchange = await db.fetchrow(
            "SELECT id FROM portfolio_service.exchanges WHERE id = $1", 
            exchange_id
        )
        ...
        # Delete exchange
        await db.execute(
                "DELETE FROM portfolio_service.exchanges WHERE id = $1",
                exchange_id
            )
